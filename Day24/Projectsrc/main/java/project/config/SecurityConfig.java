...
public class SecurityConfig {
	...
	SecurityFilterChain securityFilterChain(HttpSecurity security) throws Exception {
		security.cors(corsConfig -> corsConfig.configurationSource(corsConfiger())).csrf(csrf -> csrf.disable())
				.authorizeHttpRequests(
						req -> req.requestMatchers("/ignore/**").authenticated().requestMatchers("/**").permitAll())
